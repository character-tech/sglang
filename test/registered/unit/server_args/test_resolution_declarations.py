...
class TestDeclaredValuesAreNotEditedLater(CustomTestCase):
    ...
    def _resolve_recording_each_entry(self, **supplied):
        ...
        # Every path that appends: the record's own handlers reach
        # `declare_resolution` through `_declare`, the launcher stage uses
        # `declare_late_resolution`, the legacy writers `declare_direct_writes`,
        # and a pass slot appends from `run_post_process_pass`.
        patched = {}
        for name in (
            "declare_resolution",
            "declare_late_resolution",
    ...
    def test_no_entry_changes_after_it_is_recorded(self):
        ...
        for label, supplied in (
            ...
            # DP attention clamps the prefill graph, and an encoder-style
            # prefill raises it: two handlers that reach the config through a
            # local alias, which the first pass of this conversion missed.
