...
class Scheduler(
    ...
    def __init__(
        ...
        self.ps = ParallelState(
            ...
            attn_cp_size=get_parallel().configured_attn_cp_size,
            attn_dcp_rank=tp_rank % get_parallel().configured_dcp_size,
            attn_dcp_size=get_parallel().configured_dcp_size,
...
def dispatch_event_loop(scheduler: Scheduler):
    ...
    if disaggregation_mode == DisaggregationMode.NULL:
        ...
        elif get_parallel().configured_pp_size > 1:
        ...
        if get_parallel().configured_pp_size > 1:
...
def configure_scheduler_process(
    ...
    if get_parallel().configured_pp_size > 1:
        prefix += f" PP{pp_rank}"
    if get_parallel().configured_attn_cp_size > 1:
        prefix += f" ATTN_CP{attn_cp_rank}"
    if get_parallel().configured_moe_dp_size > 1:
        prefix += f" MOE_DP{moe_dp_rank}"
    if get_parallel().configured_tp_size > 1:
